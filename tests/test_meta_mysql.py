...
def test_mysql_meta_database_initializes_platform_tables_and_defaults(tmp_path):
    ...
    assert source["database_url"] == DEFAULT_MYSQL_DATA_SOURCE_URL
    assert source["allowed_tables"] == ["orders", "users", "products"]
    assert len(db.list_metrics()) == len(DEFAULT_METRICS)


def test_mysql_meta_database_data_source_crud(tmp_path):
...
def test_mysql_meta_database_metric_crud(tmp_path):
    db, _fake = _db(tmp_path)
    db.initialize()
