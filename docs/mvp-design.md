...
- 输出三张表结构、字段说明和关联关系
- 默认用 SQLite 保存平台元数据；`META_DB_TYPE=mysql` 时切到 MySQL 元数据库
...
- 最近一次真实运行（2026-07-20）结果为 `off 3/3`、`on 3/3`、`Delta: tie`，尚未证明质量提升。
...
2. MySQL 元数据库已支持兼容切换；需要时运行迁移脚本把 SQLite 平台元数据导入 MySQL。
3. 迁移和回归验证完成后，再删除 SQLite 默认运行依赖，将 PostgreSQL 调整为可选能力。
