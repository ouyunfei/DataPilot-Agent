...
- 语义层：用 SQLite 配置销售额、退款率、客单价、毛利、订单数等业务指标口径。
...
位置：`app/db/database.py`、`app/services/semantic.py`
...
启动时自动初始化默认指标。`build_semantic_context()` 会读取启用指标并拼接进 schema 上下文，让 DeepSeek 生成 SQL 时能看到统一业务口径。数据源表白名单和字段白名单会过滤引用了无权限表或无权限字段的指标。
